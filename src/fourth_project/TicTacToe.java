...
public class TicTacToe implements Initializable {
	...
	@FXML
	private ComboBox<String> comboFirstPlayer;
	...
	@FXML
	private TextArea minMaxTextArea;

	@FXML
	private Rectangle rectangle00, rectangle01, rectangle02, rectangle10, rectangle11, rectangle12, rectangle20,
			rectangle21, rectangle22;
	...
	public void initialize(URL url, ResourceBundle resourceBundle) {
		...
		this.comboFirstPlayer.getItems().addAll("Player", "Computer");
		this.board = new char[][] { { '-', '-', '-' }, { '-', '-', '-' }, { '-', '-', '-' } };
	...
	private void easySenario(Node node) {
		...
		// check if the game was finished by the opponent(computer) after the second
		// move
	...
	private int[] findBestMove(char[][] board) {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (board[i][j] == '-') {
					board[i][j] = this.opponent; // Assuming this is the AI's move

					int moveValue = minimax(board, 0, false);

					// Update the TextArea with the move's details

					updateMinMaxTextArea(i, j, moveValue);
	...
	private int minimax(char[][] board, int depth, boolean isMax) {
		// depth: لما يكون كمية احتمالات كبيرة
		// لما يبطل في توازن في التري بصير يفحص العمق ويحافظ على التوازن
		// 7 factorial
	...
	private void updateMinMaxTextArea(int row, int column, int score) {
	 //   minMaxTextArea.clear();
	//	startNewCalculations();

		String text = "Placing at [" + row + ", " + column + "] with score: " + score + "\n";

		minMaxTextArea.appendText(text);

	}
	
	private void startNewCalculations() {
	    minMaxTextArea.clear();
	}
	...
	private boolean isLegalMove(Node node, char whichPlayer) {
		...
		if (node.getId().equals("rectangle00")) {
			...
			if (this.board[0][0] == '-')
				this.board[0][0] = whichPlayer;
			else
				return false;// the cell was filled

		} else if (node.getId().equals("rectangle01")) {

			if (this.board[0][1] == '-')
				this.board[0][1] = whichPlayer;
			else
				return false;
	...
	private Circle drawO(double layout_X, double layout_Y) {
		...
		// Creating a gradient for the stroke
		Stop[] stops = new Stop[] { new Stop(0, Color.TEAL), new Stop(1, Color.LIGHTGREEN) };
		LinearGradient gradient = new LinearGradient(0, 0, 1, 0, true, CycleMethod.NO_CYCLE, stops);

		circle.setStroke(gradient);
	...
	private Group drawX(double layout_X, double layout_Y) {
		...
		// Gradient for the 'X' lines
		Stop[] stops = new Stop[] { new Stop(0, Color.PURPLE), new Stop(1, Color.PINK) };
		LinearGradient gradient = new LinearGradient(0, 0, 1, 0, true, CycleMethod.NO_CYCLE, stops);

		line1.setStroke(gradient);
		line1.setLayoutX(layout_X + 140);
		line1.setLayoutY(layout_Y + 165);
